        ...
        <div>
            <div className="grid grid-cols-12  h-screen sm:px-40 ">
                ...
                <div className="col-span-0 sm:col-span-4 h-full w-fit">
                    {!user ? (
                        <div className="border p-5 bg-slate-700 rounded-lg cursor-pointer transition-all">
                        ...
                        <div className="col-span-0 sm:col-span-4 h-full w-fit">
                            {user?.recommendedUsers?.map((el) => (
                                <Link href={`${el?.id}`}><div key={el?.id}>{el?.profileImageURL && <Image src={el?.profileImageURL} alt="userImage" width={50} height={50} />}<span>{el?.firstName} {el?.lastName}</span></div></Link>
                            ))
                            }
                        </div>
                    )}
